...
sphere = Entity(model='sphere', color=color.black, scale=sphere_scale, position=(0,0,0), dz=1e+10)
# create photon ring
PhotonRing = Entity(model=Circle(resolution=50, radius=radius/1.9, mode="line", thickness=2),
...
for numArcs in range(n):
    ...
    bottomArcs.append(Entity(model=Circle(resolution=1000, radius=arcRadius, mode="line", thickness=2),
...
for numArcs in range(n):
    ...
    vertices = []
    # loop through a set of possible y values of a circle
    for y in np.linspace(0, -arcRadius, 1000):
        x = np.sqrt((arcRadius**2) - (y)**2)
        # positive square root answer
        vertices.append([x, y, 0])
    # add negative square root answers
    ...
    # convert to a tuple
    vertices = tuple([tuple(i) for i in vertices])
    accArcs.append(Entity(model=Mesh(vertices=vertices, mode='line', thickness=2), color=color.rgba(np.linspace(255,237,n)[numArcs],
                                                                                          np.linspace(252,130,n)[numArcs],
                                                                                          np.linspace(200,54,n)[numArcs],
                                                                                          255-numArcs),
                          rotation_x=90))

    accArcs.append(Entity(parent=accArcs[-1], model=Mesh(vertices=vertices, mode='line', thickness=2),
                                         color=color.rgba(np.linspace(255,237,n)[numArcs],
                                                                                          np.linspace(252,130,n)[numArcs],
                                                                                          np.linspace(200,54,n)[numArcs],
                                                                                          255-numArcs),
                          rotation_x=90))
    hitinfo = accArcs[-1].intersects()
    if hitinfo.hit:
        accArcs[-1].combine()
        print("top arc intersects")

    # increase arc radius using a nonlinear spaced vector of numbers
    if numArcs != round(n / 1.1):
        arcRadius += (n / 5000) * (np.linspace(0, 1, n) ** 2)[numArcs]
    else:
        # significantly increase arc radius to make it look more like a black hole
        arcRadius += 0.08
