...
# create photon ring
PhotonRing = Entity(
    model=Circle(resolution=50, radius=radius / 1.99, mode="line", thickness=2),
...
for numArcs in range(n):
    ...
    bottomArcs.append(
        Entity(
            model=Mesh(vertices=vertices, mode="line"),
            color=color.rgba(
                np.linspace(255, 237, n)[numArcs],
    ...
    accArcs3.append(
        Entity(
            vertices=tuple(
                set(
                    Entity(
                        model=Mesh(vertices=vertices, mode="line", thickness=20),
                        rotation_x=90,
                        color=color.rgba(
                            np.linspace(255, 237, n)[numArcs],
                            np.linspace(252, 130, n)[numArcs],
                    ...
                    + Entity(
                        model=Mesh(vertices=vertices2, mode="line", thickness=20),
                        color=color.rgba(
                            np.linspace(255, 237, n)[numArcs],
                            np.linspace(252, 130, n)[numArcs],
                            np.linspace(200, 54, n)[numArcs],
